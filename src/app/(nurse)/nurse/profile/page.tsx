...
import { Upload, Lock, Stethoscope } from 'lucide-react'
...
export default function NurseProfilePage() {
  ...
  const assignedDoctor = useMemo(() => staff.find((s) => s.id === nurse.assignedDoctorId), [nurse])
  ...
  return (
    <motion.div className="space-y-6" initial="hidden" animate="visible" variants={stagger}>
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="relative">
              {avatarPreview ? (
                ...
                <UserAvatar name={nurse.name} size="lg" className="h-20 w-20 text-2xl" />
              )}
              <label
                htmlFor="av-file"
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-ink-soft">Nurse &middot; {nurse.id}</p>
              <p className="text-xs text-ink-faint">{nurse.qualifications}</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div variants={fadeUp}>
        <SectionCard title="Assigned Doctor">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-ink">{assignedDoctor?.name ?? '\u2014'}</p>
              <p className="text-xs text-ink-faint">{assignedDoctor?.specialty ?? 'General Medicine'} &middot; {assignedDoctor?.email}</p>
            </div>
          </div>
        </SectionCard>
      </motion.div>
